...
class TestCreateReplacementLambda:
    ...
    def test_create_replacement_lambda_basic(self):
        ...
        with patch('src.embedded_cereal_bowl.monitor.monitor.colour_str') as mock_colour:
            mock_coloured = Mock()
    ...
    def test_create_replacement_lambda_with_ansi_codes(self):
        ...
        with patch('src.embedded_cereal_bowl.monitor.monitor.colour_str') as mock_colour:
            ...
            mock_match.end.return_value = len("normal text \x1b[31mred text\x1b[0m error")
...
class TestSendSerialData:
    ...
    def test_send_serial_data_no_file(self):
        ...
        with patch('src.embedded_cereal_bowl.monitor.monitor.add_time_to_line', mock_add_time):
    ...
    def test_send_serial_data_serial_exception(self, mock_add_time, mock_colour):
        ...
        mock_ser = Mock()
        mock_ser.write.side_effect = Exception("Serial error")
        mock_file = Mock()
        mock_add_time.return_value = "timestamp test data\n"
        mock_colour.return_value = "colored error"
    ...
    def test_send_serial_data_no_file(self):
        """Test sending data without logging file."""
        mock_ser = Mock()
        mock_add_time = Mock(return_value="timestamp test data\n")
...
class TestWaitWithSpinner:
    ...
    def test_wait_with_spinner_basic(self, mock_colour, mock_flush, mock_write):
        ...
        mock_colour.return_value = "spinner text"
...
class TestRunSerialPrintingWithLogs:
    ...
    @patch('os.path.isdir')
    @patch('os.mkdir')
    def test_run_serial_printing_with_logs_new_directory(self, mock_mkdir, mock_isdir, mock_datetime, mock_run):
        ...
        with patch('builtins.open', mock_open()) as mock_file:
            run_serial_printing_with_logs("/dev/ttyUSB0", 115200, "test", "/tmp/logs", "epoch", None, False)
            
            mock_isdir.assert_called_once_with("/tmp/logs")
            mock_mkdir.assert_called_once_with("/tmp/logs")
            mock_file.assert_called_once_with("/tmp/logs/2023.01.01_12.00.00_test.txt", "a+", buffering=1)
    ...
    @patch('os.path.isdir')
    @patch('os.mkdir')
    def test_run_serial_printing_with_logs_existing_directory(self, mock_mkdir, mock_isdir, mock_datetime, mock_run):

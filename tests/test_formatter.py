...
class TestFindAllFiles:
    ...
    def test_find_all_files_basic(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected_files = {root / "test.cpp", root / "test.h", root / "CMakeLists.txt"}
    ...
    def test_process_one_file_with_changes(self):
        ...
        try:
            ...
            with patch('subprocess.run') as mock_run:
                ...
                with patch.object(temp_file, 'read_bytes') as mock_read:
            ...
            with patch('subprocess.run') as mock_run:
                ...
                with patch.object(temp_file, 'read_bytes') as mock_read:
                    ...
                    def read_side_effect():
                        return changed_content if mock_run.call_args[0][0][-1] == "-i" else original_content
...
class TestRunProjectTasks:
    ...
    def test_run_project_tasks_no_files(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('src.embedded_cereal_bowl.formatter.formatter.find_all_files') as mock_find:
                mock_find.return_value = {}
                
                with patch('src.embedded_cereal_bowl.formatter.formatter.process_files_parallel') as mock_process:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            with patch('src.embedded_cereal_bowl.formatter.formatter.find_all_files') as mock_find:
                ...
                with patch('src.embedded_cereal_bowl.formatter.formatter.process_files_parallel') as mock_process:
                    ...
                    mock_process.assert_called_once_with(mock_files, root, 4, True, True)
...
class TestInterfaceFunctions:
    ...
    def test_format_files(self):
        ...
        with patch('src.embedded_cereal_bowl.formatter.formatter.check_for_tools') as mock_check:
            with patch('src.embedded_cereal_bowl.formatter.formatter.run_project_tasks') as mock_run:

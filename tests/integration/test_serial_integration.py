...
class TestHandleUserInputIntegration:
    ...
    def test_handle_user_input_normal_operation(self, mock_send, mock_readline, mock_select):
        ...
        with patch('threading.Event.wait', side_effect=[False, True]):  # Stop after first iteration
            try:
    ...
    def test_handle_user_input_empty_line(self, mock_readline, mock_select):
        ...
        with patch('src.embedded_cereal_bowl.monitor.monitor.send_serial_data') as mock_send:
...
class TestSerialLoopIntegration:
    ...
    def test_serial_loop_with_send_enabled(self, mock_colour, mock_thread, mock_send):
        ...
        with patch('builtins.print') as mock_print:
            with patch('src.embedded_cereal_bowl.monitor.monitor.add_time_to_line', return_value=""):
                try:
...
class TestRunSerialPrintingIntegration:
    ...
    def test_run_serial_printing_successful_connection(self, mock_sleep, mock_loop):
        ...
        with patch('serial.Serial') as mock_serial:
            ...
            with patch('builtins.print'):
                with patch('src.embedded_cereal_bowl.monitor.monitor.wait_with_spinner'):
                    try:
    ...
    @patch('time.sleep')
    @patch('src.embedded_cereal_bowl.monitor.monitor.colour_str')
    def test_run_serial_printing_connection_failure_retry(self, mock_colour, mock_sleep, mock_spinner):
    ...
    def test_run_serial_printing_keyboard_interrupt(self, mock_colour, mock_exit):
        ...
        with patch('src.embedded_cereal_bowl.monitor.monitor.serial.Serial', mock_serial):
            with patch('src.embedded_cereal_bowl.monitor.monitor.wait_with_spinner', return_value=1):
                run_serial_printing("/dev/ttyUSB0", 115200, "epoch", None, None, False)
    ...
    @patch('src.embedded_cereal_bowl.monitor.monitor.wait_with_spinner')
    @patch('time.sleep')
    def test_run_serial_printing_with_highlight_words(self, mock_sleep, mock_spinner, mock_loop):
        ...
        with patch('builtins.print') as mock_print:
            with patch('src.embedded_cereal_bowl.monitor.monitor.serial.Serial', mock_serial):
                try:
                    run_serial_printing("/dev/ttyUSB0", 115200, "epoch", None, highlight_words, False)
    ...
    @patch('src.embedded_cereal_bowl.monitor.monitor.wait_with_spinner')
    @patch('time.sleep')
    def test_run_serial_printing_with_send_enabled(self, mock_sleep, mock_spinner, mock_loop):

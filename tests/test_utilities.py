...
class TestUtilityFunctions:
    ...
    def test_run_serial_printing_with_logs_new_dir(self, mock_run, mock_open, mock_isdir, mock_mkdir):
        """Test logging with new directory creation."""
        mock_isdir.return_value = False
        
        with patch('src.embedded_cereal_bowl.monitor.monitor.datetime') as mock_datetime:
        ...
        mock_open.assert_called_once_with("/tmp/logs/2023.01.01_12.00.00_test.txt", "a+", buffering=1)
    ...
    @patch('builtins.open')
    @patch('src.embedded_cereal_bowl.monitor.monitor.run_serial_printing')
    def test_run_serial_printing_with_logs_existing_dir(self, mock_run, mock_open, mock_isdir):
...
class TestSerialLoopEdgeCases:
    ...
    def test_serial_loop_with_highlight_processing(self):
        ...
        mock_ser = Mock()
        mock_ser.readline.side_effect = [b"This is an error message\n", KeyboardInterrupt()]
        
        with patch('builtins.print') as mock_print:
            with patch('src.embedded_cereal_bowl.monitor.monitor.add_time_to_line', return_value=""):
                with patch('src.embedded_cereal_bowl.monitor.monitor.create_replacement_lambda') as mock_replacement:
...
class TestErrorHandling:
    ...
    def test_handling_decode_errors(self):
        ...
        mock_ser.readline.side_effect = [b"valid data\n", b"\xff\xfe invalid\n", KeyboardInterrupt()]
